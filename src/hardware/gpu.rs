...
pub(crate) type GpuPool = Arc<Mutex<GpuManager>>;
...
impl GpuManager {
    ...
    pub(crate) fn new(gpu_indices: Vec<usize>, threshold: u32) -> GpuPool {
    ...
    /// * `gpus` - GPU index list
    /// * `pid` - Process ID
    pub(crate) fn update_pid_batch(&mut self, gpus: &[usize], pid: u32) {
        for &idx in gpus {
    ...
    pub(crate) fn get_all_status(&self) -> Vec<GpuInfo> {
        self.gpus.clone()
    }
    ...
    pub(crate) fn find_gpu_ids_by_state(&self, state: GpuState) -> Vec<usize> {
        self.gpus
            .iter()
    ...
    pub(crate) fn find_available_gpus(&self, count: usize) -> Result<Vec<usize>, String> {
    ...
    /// * `job_id` - Job ID
    /// * `gpus` - GPU index list
    pub(crate) fn allocate_batch(&mut self, job_id: usize, gpus: &[usize]) {
        for &idx in gpus {
    ...
    ///
    /// * `gpus` - GPU index list
    pub(crate) fn release_batch(&mut self, gpus: &[usize]) {
        for &idx in gpus {
            self.release(idx);
        }
    ...
    pub(crate) fn get_first_pid_by_job_id(&self, job_id: usize) -> Option<u32> {
        self.gpus
            .iter()
...
pub(crate) fn get_gpu_info() -> Vec<GpuPrintInfo> {
